...
class Solution:
    def matchGame(self, N):
        # Check if N is a losing position for player A
        if N % 5 == 0:
            return -1  # Impossible for A to win
        else:
            return N % 5  # Number of matchsticks A should pick

# Driver Code
if _name_ == '_main_':
    t = int(input())  # Number of test cases
    for _ in range(t):
        N = int(input())  # Number of matchsticks
        ob = Solution()
